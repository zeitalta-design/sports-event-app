/**
 * Sports Entry 一覧データを SQLite へ upsert
 */
...
function importEvents(events, options = {}) {
  ...
  const existingStmt = db.prepare(
    "SELECT id, source_event_id FROM events WHERE source_site = 'sportsentry' AND source_event_id = ?"
  );
  ...
  const updateStmt = db.prepare(`
    ...
    WHERE source_site = 'sportsentry' AND source_event_id = @source_event_id
  `);
  ...
  const upsertAll = db.transaction(() => {
    for (const ev of events) {
      const row = {
        source_site: ev.source_site || "sportsentry",
  ...
  const total = db.prepare("SELECT COUNT(*) as c FROM events WHERE source_site = 'sportsentry'").get().c;
