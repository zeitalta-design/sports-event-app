...
export async function PUT(request, { params }) {
  try {
    ...
    const details = { domain: "shitei", slug: item.slug, title: item.title };
    if (before && before.is_published !== item.is_published) details.is_published_changed = `${before.is_published} → ${item.is_published}`;
    writeAuditLog({ userId: guard.user.id, action: AUDIT_ACTIONS.ADMIN_ITEM_UPDATED, targetType: "shitei_item", targetId: String(numId), details, ipAddress, userAgent });
