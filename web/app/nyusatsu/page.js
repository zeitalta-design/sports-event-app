      ...
      renderFilters={() => (
        <div className="card p-4 mb-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="text" value={keyword} onChange={(e) => { setKeyword(e.target.value); setPage(1); }} placeholder="案件名・発注機関で検索..." className="flex-1 border rounded-lg px-4 py-2.5 text-sm" />
            <select value={category} onChange={(e) => { setCategory(e.target.value); setPage(1); }} className="border rounded-lg px-3 py-2.5 text-sm">
              ...
              {nyusatsuConfig.categories.map((cat) => (
