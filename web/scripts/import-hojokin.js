 ...
 *
 * Usage:
 *   node scripts/import-hojokin.js <input.json> [options]
 *   node scripts/import-hojokin.js --remote <URL> [options]
 *   node scripts/import-hojokin.js --source <name> [options]
 *
 * Source env: --source primary → HOJOKIN_SOURCE_PRIMARY_URL
 */
...
const { loadJson, loadRemoteJson, runImport } = await import(pathToFileURL(resolve(__dirname, "../lib/importers/hojokin.js")).href);
const { fetchFromSource, getAvailableSources } = await import(pathToFileURL(resolve(__dirname, "../lib/importers/hojokin-source.js")).href);
const { getDb } = await import(pathToFileURL(resolve(__dirname, "../lib/db.js")).href);

function upsertHojokinItem(item) {
  const db = getDb();
  const existing = item.slug ? db.prepare("SELECT id FROM hojokin_items WHERE slug = ?").get(item.slug) : null;
  if (existing) {
    db.prepare(`UPDATE hojokin_items SET title=@title, category=@category, target_type=@target_type, max_amount=@max_amount, subsidy_rate=@subsidy_rate, deadline=@deadline, status=@status, provider_name=@provider_name, summary=@summary, is_published=@is_published, updated_at=datetime('now') WHERE id=@id`).run({ ...item, id: existing.id });
    return { action: "update", id: existing.id };
  }
  const r = db.prepare(`INSERT INTO hojokin_items (slug, title, category, target_type, max_amount, subsidy_rate, deadline, status, provider_name, summary, is_published, created_at, updated_at) VALUES (@slug, @title, @category, @target_type, @max_amount, @subsidy_rate, @deadline, @status, @provider_name, @summary, @is_published, datetime('now'), datetime('now'))`).run(item);
...
const report = runImport(rawItems, { dryRun, upsertFn: upsertHojokinItem, verbose });
...
else { try { const c = getDb().prepare("SELECT COUNT(*) as c FROM hojokin_items").get(); console.log(`\n📊 hojokin_items 合計: ${c.c} 件`); } catch {} }
console.log();
