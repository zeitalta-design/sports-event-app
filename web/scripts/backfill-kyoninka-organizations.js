...
async function main() {
  ...
  if (!DRY_RUN) {
    ...
    const crossDomain = db.prepare(`
      ...
      FROM organizations o
      WHERE (SELECT COUNT(*) FROM kyoninka_entities k2 WHERE k2.organization_id = o.id)
          + (SELECT COUNT(*) FROM administrative_actions a2 WHERE a2.organization_id = o.id)
          + (SELECT COUNT(*) FROM hojokin_items h2 WHERE h2.organization_id = o.id) > 0
