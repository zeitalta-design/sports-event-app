...
const EMAIL_TEMPLATES = {
  deadline_today: {
    subject: (n, ev) => `【スポ活】本日締切: ${ev?.title || n.title}`,
    body: (n, ev) => buildDeadlineBody(n, ev, "本日がエントリー締切です。"),
  },
  deadline_3d: {
    subject: (n, ev) => `【スポ活】締切間近: ${ev?.title || n.title}`,
...
function buildDeadlineBody(n, ev, message) {
  ...
  lines.push("", "---", "スポ活 — スポーツ大会検索・通知サービス", APP_BASE_URL);
  return lines.join("\n");
}
