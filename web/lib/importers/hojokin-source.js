/**
 * 補助金ナビ — 実データソース adapter
 */

const SOURCES = {
  primary: { name: "primary", label: "補助金 主要データソース", envUrl: "HOJOKIN_SOURCE_PRIMARY_URL", envToken: "HOJOKIN_SOURCE_PRIMARY_TOKEN" },
};
...
export function resolveSourceConfig(sourceName) {
  ...
  if (!url) throw new Error(`環境変数 ${s.envUrl} が未設定です\n設定例: ${s.envUrl}=https://example.com/api/hojokin.json`);
...
function adaptFields(raw) {
  return {
    title: raw.title || raw.name || raw.program_name || raw.subsidy_name || null,
    slug: raw.slug || null,
    category: raw.category || raw.subsidy_category || raw.support_type || null,
    target_type: raw.target_type || raw.target || raw.eligible_entity || raw.applicant_type || null,
    max_amount: raw.max_amount || raw.maximum_amount || raw.upper_limit || null,
    subsidy_rate: raw.subsidy_rate || raw.rate || raw.support_rate || null,
    deadline: raw.deadline || raw.application_deadline || raw.end_date || null,
    provider_name: raw.provider_name || raw.provider || raw.agency || raw.organization || null,
