...
function DeadlineBadge({ days }) {
  ...
  return (
    <span
      className={`absolute top-3 left-3 z-10 px-2 py-1 text-[10px] font-bold
                  text-white rounded-md shadow-md ${bg}`}
    >
      {text}
    </span>
  );
  ...
  return (
    <Link
      ...
      className="group flex-shrink-0 block
                 w-[280px] sm:w-[calc(50%-8px)] lg:w-[calc(25%-12px)]"
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* 右上: 受付ステータス */}
          <span
            className={`absolute top-3 right-3 px-2 py-0.5 text-[10px] font-semibold rounded
                        ${isOpen ? "bg-emerald-600 text-white" : "bg-gray-600/70 text-white"}`}
          >
            {statusLabel}
          </span>
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {event.sport_type &&
              event.sport_type !== "marathon" &&
              (() => {
                const sport = SPORT_CONFIGS.find(
                  (s) => s.sportTypeForDb === event.sport_type
                );
                return sport ? (
                  <span
                    className="inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded"
                    style={{
                      backgroundColor: `${sport.themeColor}12`,
          ...
          <h3
            className="font-bold text-[15px] leading-snug line-clamp-2
                       group-hover:text-blue-700 transition-colors min-h-[2.6rem]"
            style={{ color: "#1a1a1a" }}
          >
            {event.title}
          </h3>
          ...
          {/* 開催日 — やや小さめだが視認しやすく */}
          <div className="mt-2.5 flex items-center gap-1.5 text-xs font-medium" style={{ color: "#444" }}>
            <svg
          ...
          {/* 開催地 — 補助情報として少し薄め */}
          <div className="mt-1 flex items-center gap-1.5 text-[11px]" style={{ color: "#888" }}>
            <svg
              className="w-3.5 h-3.5 flex-shrink-0"
              fill="none"
              viewBox="0 0 24 24"
              ...
              {distances.slice(0, 3).map((d) => (
                <span
                  ...
                  className="inline-block px-1.5 py-0.5 text-[10px] font-medium
                             bg-gray-50 text-gray-500 rounded border border-gray-100"
                >
                  {d}
                </span>
              ))}
          ...
          <div className="mt-auto pt-3 flex items-center justify-between border-t border-gray-50">
            <span className="inline-flex items-center gap-1 text-xs font-medium text-gray-500 group-hover:text-blue-600 transition-colors">
              詳細を見る
              <svg
