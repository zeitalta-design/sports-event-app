...
export default function TopCategoryLinks() {
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-12">
      <h2 className="text-lg font-bold mb-6" style={{ color: "#1a1a1a" }}>ジャンルから探す</h2>
        ...
        {categories.map((cat) => {
          const content = (
            ...
            >
              ...
              <h3 className="font-bold text-sm" style={{ color: "#1a1a1a" }}>{cat.label}</h3>
              <p className="text-[11px] font-medium mt-1" style={{ color: "#1a1a1a" }}>{cat.description}</p>
              {!cat.enabled && (
